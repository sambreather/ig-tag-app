...
// Delete a whole album: soft-delete, giving it (and its still-active
// videos) the same 30-day grace period individual videos get. The videos
// are flagged deletedWithAlbum so restoring the album brings back only the
// ones that went with it - not anything a colleague had already deleted
...
router.delete('/albums/:albumId', (req, res) => {
  ...
  data.videos.forEach(v => {
    if (v.albumId === album.id && !v.deleted) {
      v.deleted = true;
      v.deletedAt = now;
      v.deletedWithAlbum = true;
    }
  });
...
// Restores a deleted album and whichever of its videos were deleted along
// with it (not ones a colleague had already deleted separately).
router.post('/albums/:albumId/restore', (req, res) => {
  const data = db.load();
  const album = data.albums.find(a => a.id === req.params.albumId);
  if (!album) return res.status(404).json({ error: 'Album not found' });
  ...
  data.videos.forEach(v => {
    if (v.albumId === album.id && v.deletedWithAlbum) {
      v.deleted = false;
      v.deletedAt = null;
      v.deletedWithAlbum = false;
    }
  });
